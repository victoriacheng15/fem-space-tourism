...
const Nav = () => {
  ...
  return (
    <nav className="fixed top-0 z-50 flex items-center justify-end w-full h-auto -translate-x-1/2 flex-end left-1/2 md:h-24 max-w-screen-bpCustom">
      ...
      <ul className="items-center hidden w-auto h-full px-12 md:flex bg-white/20 gap-9 lg:px-32">
        {
          linkList.map((link) => (
            <li key={link.id} className="h-full text-2xl">
              <Link
                id={link.id}
                to={link.href}
                className="relative flex items-center h-full tracking-wider text-white uppercase font-barlow content-none link-hover"
              >
